...
END;
    ...
    createAccount(account) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(account.id, account.name, account.phone, account.imei, account.cookie, account.user_agent, account.status, account.last_login);
    ...
    createFriend(friend) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(friend.id, friend.account_id, friend.name, friend.display_name, friend.phone, friend.avatar, friend.status, friend.last_interaction);
    ...
    updateFriend(id, accountId, updates) {
        const fields = Object.keys(updates)
            .map(key => `${key} = ?`)
            .join(', ');
        if (fields.length === 0)
    ...
    removeFriendTag(friendId, accountId, tag) {
        const stmt = this.db.prepare(`
      DELETE FROM friend_tags WHERE friend_id = ? AND account_id = ? AND tag = ?
    `);
        stmt.run(friendId, accountId, tag);
    }
    getFriendTags(friendId, accountId) {
        const stmt = this.db.prepare(`
      SELECT tag FROM friend_tags WHERE friend_id = ? AND account_id = ?
    `);
        const rows = stmt.all(friendId, accountId);
        return rows.map(r => r.tag);
    }
    getAllTagsByAccount(accountId) {
        const stmt = this.db.prepare(`
    ...
    createGroup(group) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(group.id, group.account_id, group.name, group.description, group.avatar, group.member_count, group.is_admin, group.last_activity, group.type);
    ...
    updateGroup(id, accountId, updates) {
        const fields = Object.keys(updates)
            .map(key => `${key} = ?`)
            .join(', ');
        if (fields.length === 0)
    ...
    createTemplate(template) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(template.id, template.name, template.content, template.type, template.category, template.variables, template.media);
    ...
    // ==================== MESSAGE LOGS ====================
    createMessageLog(log) {
        const stmt = this.db.prepare(`
      INSERT INTO message_logs (id, account_id, recipient_id, recipient_type, content, status, error)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(log.id, log.account_id, log.recipient_id, log.recipient_type, log.content, log.status, log.error);
    ...
    getMessageLogsByAccount(accountId, limit = 100) {
        const stmt = this.db.prepare(`
    ...
    // ==================== SHARE CONTENT ====================
    createShareContent(content) {
        const stmt = this.db.prepare(`
      INSERT INTO share_content (id, title, content, category_id, media)
      VALUES (?, ?, ?, ?, ?)
    `);
        stmt.run(content.id, content.title, content.content, content.category_id, content.media);
